...
## zwst_field_fixtures.jl

Stationary (ISO 532-1:2017 clause 5, "Method 1") free- vs diffuse-field
oracle. Generated by `scripts/generate_mosqito_zwst_field_crosscheck.jl`
+ `scripts/crosscheck_zwst_field.py` from MoSQITo @ commit `d990c33f94f1`
(<https://github.com/Eomys/MoSQITo>, Apache License 2.0), calling its
private stationary stages `_main_loudness` + `_calc_slopes` on four
synthetic 28-band SPL vectors, once per field type.

- `ZWST_FIELD_FIXTURE_CASES`: per case, the input `spl` vector and, for
  both `free` and `diffuse`, total loudness `N` [sone] and 240-bin
  `N_specific` [sone/Bark]. Not derived from any ISO-copyrighted material.
